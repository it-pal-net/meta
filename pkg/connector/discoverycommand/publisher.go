// Package discoverycommand publishes "run external-conversation discovery"
// commands to the SyncContact chat-command stream (stream:commands) so the API
// surfaces new inbound conversations live, without waiting for the operator to
// reload / re-navigate. Mirrors the WhatsApp bridge's discoverycommand package;
// the API resolves the command by (ownerMatrixUserId, bridgeLoginId) regardless
// of provider.
